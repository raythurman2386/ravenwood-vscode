...
import type { Palette, SyntaxRule } from '../interface';

/** Palette key referenced by a rule's `foreground`. */
type PaletteKey = keyof Palette;

/** Rule definition with optional flags controlling behaviour under `italicKeywords`. */
export interface SyntaxRuleDef {
  name: string;
  scope: string;
  settings: {
    /** Palette key resolved to a hex colour by buildSyntax at runtime. */
    foreground?: PaletteKey;
    fontStyle?: string;
  };
  /** When true, this rule's fontStyle gets 'italic' appended if italicKeywords is true. */
  italicizeKeywords?: boolean;
  /** When true, this rule is only emitted when italicKeywords is true. */
  onlyWhenItalicKeywords?: boolean;
  /** When true, this rule is only emitted when italicKeywords is false. */
  onlyWhenNotItalicKeywords?: boolean;
  /** When set, overrides `scope` when italicKeywords is true. */
  italicKeywordsScope?: string;
}
...
export const SYNTAX_RULES: SyntaxRuleDef[] = [
  ...
  },
  {
    name: 'Stylus grey',
    scope:
      'punctuation.definition.entity.stylus, punctuation.separator.key-value.stylus, punctuation.terminator.rule.stylus, punctuation.separator.list.comma.stylus, punctuation.definition.parameters.begin.stylus, punctuation.definition.parameters.end.stylus',
    settings: { foreground: 'grey1' },
  },
  {
    name: 'Stylus red',
    scope:
      'keyword.control.stylus, keyword.control.at-rule.stylus, keyword.control.conditional.stylus',
    settings: { foreground: 'red' },
    italicizeKeywords: true,
  },
  {
    name: 'Stylus orange',
    scope:
      'keyword.other.unit.stylus, keyword.operator.stylus, support.type.vendored.property-name.stylus',
    settings: { foreground: 'orange' },
  },
  {
    name: 'Stylus yellow',
  ...
  },
  {
    name: 'Stylus green',
    scope:
      'string.quoted.single.stylus, string.quoted.double.stylus, support.constant.property-value.stylus, constant.numeric.stylus, punctuation.definition.string.begin.stylus, punctuation.definition.string.end.stylus',
    settings: { foreground: 'green' },
  },
  {
    name: 'Stylus aqua',
    scope:
      'support.type.property-name.stylus, entity.other.attribute-name.stylus',
    settings: { foreground: 'aqua' },
  },
  {
    name: 'Stylus blue',
    scope: 'variable.other.stylus, support.type.variable.stylus',
    settings: { foreground: 'blue' },
  },
  {
    name: 'Stylus purple',
    scope:
      'entity.name.tag.stylus, entity.other.keyframe-offset.stylus, punctuation.definition.keyword.stylus, keyword.control.at-rule.import.stylus, keyword.control.at-rule.include.stylus, meta.selector.stylus',
    settings: { foreground: 'purple' },
    italicizeKeywords: true,
  },
  {
  ...
  },
  {
    ...
    settings: { foreground: 'red' },
    italicizeKeywords: true,
    italicKeywordsScope: 'keyword.control.go, keyword.type.go',
  },
  {
    name: 'Go orange',
    scope:
  ...
  {
    name: 'Swift orange',
    scope: 'storage.type.swift, storage.modifier.swift, keyword.operator.swift',
    settings: { foreground: 'orange' },
    italicizeKeywords: true,
  },
  {
  ...
  },
  {
    name: 'Python yellow',
    scope:
      'entity.name.type.python, support.type.python, entity.name.function.decorator.python, entity.name.function.decorator.python, meta.function.decorator.python',
    settings: { foreground: 'yellow' },
    italicKeywordsScope:
  ...
  },
  {
    ...
    settings: { foreground: 'purple' },
    onlyWhenNotItalicKeywords: true,
  },
  {
  ...
  {
    name: 'Erlang blue',
    scope: 'entity.name.type.erlang, support.type.erlang, keyword.type.erlang',
    settings: { foreground: 'blue' },
  },
  {
    name: 'Erlang purple',
  ...
  {
    name: 'GraphQL grey',
    scope: 'punctuation.definition.type.graphql, punctuation.separator.graphql',
    settings: { foreground: 'grey1' },
  },
  {
    name: 'GraphQL orange',
...
export function buildSyntax(
  ...
  for (const def of SYNTAX_RULES) {
    ...
    if (def.onlyWhenNotItalicKeywords && italicKeywords) continue;

    const scope =
      italicKeywords && def.italicKeywordsScope !== undefined
        ? def.italicKeywordsScope
        : def.scope;

    let fontStyle = def.settings.fontStyle;
    if (italicKeywords && def.italicizeKeywords) {
      fontStyle = fontStyle ? `${fontStyle} italic` : 'italic';
    }

    const settings: { foreground?: string; fontStyle?: string } = {};
    if (def.settings.foreground !== undefined) {
      settings.foreground = palette[def.settings.foreground];
    }
